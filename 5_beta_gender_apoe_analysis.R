...
# untargeted gender beta analysis ----
gender_res <- beta_analysis(D=Dmc, outcomes=outcomes, groups_variable='msex', 
...
names(gender_res) <- outcomes$outcome
...
# targeted results
gender_specific <- apoe_specific <- list()
# reading metadata
res <- overall_res %>%
  excel_sheets() %>%
  purrr::set_names() %>%
...
for(outcome in outcomes$outcome){
  this_df <- res[[outcome]]
  gender_specific[[outcome]] <- gender_res[[outcome]] %>% 
